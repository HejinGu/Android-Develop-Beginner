    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mFab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent = new Intent(TodoListActivity.this, NoteActivity.class);
            ...
            public boolean onLongClick(View v) {
                new Thread() {
                    ...
                    public void run() {
                        ...
                        dao.deleteAll();
                        for (int i = 0; i < 20; ++i) {
                            dao.addTodo(new TodoListEntity("This is " + i + " item",
        ...
        if (requestCode == REQUEST_CODE_CREATE_ITEM) {
            if (RESULT_OK == resultCode && null != data) {
                new Thread(){
                    @Override
                    public void run() {
                        String item = data.getStringExtra(NoteActivity.KEY);
                        TodoListDao dao = TodoListDatabase.inst(TodoListActivity.this).todoListDao();
                        dao.addTodo(new TodoListEntity(item,new Date(System.currentTimeMillis()),0L));
                        Snackbar.make(mFab, R.string.insert_success, Snackbar.LENGTH_SHORT).show();
                        loadFromDatabase();
                    }
                }.start();
            }
